...
/// - Y 軸：動態刻度（對齊漏氣率的做法），單位：/min
///
/// 重要：不改動資料引擎，只吃 DailyBucket 的 rolling time series。
class Prs1ChartRespRate extends StatefulWidget {
  const Prs1ChartRespRate({
    super.key,
  ...
  static const String chartTitle = '呼吸速率';
  ...
  @override
  State<Prs1ChartRespRate> createState() => _Prs1ChartRespRateState();
}

class _Prs1ChartRespRateState extends State<Prs1ChartRespRate> {
  ...
  Widget build(BuildContext context) {
    ...
    final pts = <_Pt>[];
    for (final sm in widget.bucket.rollingRespRate5m) {
      final v = sm.value;
      if (v == null) continue;
        ...
        return Column(
          ...
          children: [
            ...
            Row(
              children: [
                const Text(
                  Prs1ChartRespRate.chartTitle,
                  style: TextStyle(fontSize: 22, fontWeight: FontWeight.w700),
                ),
                const SizedBox(width: 10),
                _LegendDot(color: Colors.purple.withOpacity(0.85)),
                const Spacer(),
              ],
            ),
                  ...
                  children: [
                    SizedBox(
                      ...
                      child: CustomPaint(
                        ...
                        painter: _AxisPainter(theme: theme, tickMin: y.tickMin, tickMax: y.tickMax, ticks: y.ticks, unit: '/min'),
                      ),
                    ),
                    Expanded(
                        ...
                        child: SingleChildScrollView(
                          ...
                          child: CustomPaint(
                            ...
                            painter: _SeriesPainter(
                              ...
                              ticks: y.ticks,
                              pts: pts,
                              lineColor: Colors.purple.withOpacity(0.85),
                            ),
                          ),
                        ),
                      ),
