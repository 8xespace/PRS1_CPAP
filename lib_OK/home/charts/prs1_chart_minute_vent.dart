...
/// - Y 軸：動態刻度（對齊漏氣率的做法），單位：L/min
///
/// 重要：不改動資料引擎，只吃 DailyBucket 的 rolling time series。
class Prs1ChartMinuteVent extends StatefulWidget {
  const Prs1ChartMinuteVent({
    super.key,
  ...
  @override
  State<Prs1ChartMinuteVent> createState() => _Prs1ChartMinuteVentState();
}

class _Prs1ChartMinuteVentState extends State<Prs1ChartMinuteVent> {
  ...
  Widget build(BuildContext context) {
    ...
    final pts = <_Pt>[];
    for (final sm in widget.bucket.rollingMinuteVent5m) {
      final v = sm.value;
      if (v == null) continue;
        ...
        return Column(
          ...
          children: [
            ...
            Row(
              children: [
                const Text(
                  Prs1ChartMinuteVent.chartTitle,
                  style: TextStyle(fontSize: 22, fontWeight: FontWeight.w700),
                ),
                const SizedBox(width: 10),
                _LegendDot(color: Colors.teal.withOpacity(0.85)),
                const Spacer(),
              ],
            ),
                  ...
                  children: [
                    SizedBox(
                      ...
                      child: CustomPaint(
                        ...
                        painter: _AxisPainter(theme: theme, tickMin: y.tickMin, tickMax: y.tickMax, ticks: y.ticks, unit: 'L/min'),
                      ),
                    ),
                    Expanded(
                        ...
                        child: SingleChildScrollView(
                          ...
                          child: CustomPaint(
                            ...
                            painter: _SeriesPainter(
                              ...
                              ticks: y.ticks,
                              pts: pts,
                              lineColor: Colors.teal.withOpacity(0.85),
                            ),
                          ),
                        ),
                      ),
