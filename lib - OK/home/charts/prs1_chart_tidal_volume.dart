...
/// - Y 軸：動態刻度（對齊漏氣率的做法），單位：mL
///
/// 重要：不改動資料引擎，只吃 DailyBucket 的 rolling time series。
class Prs1ChartTidalVolume extends StatefulWidget {
  const Prs1ChartTidalVolume({
    super.key,
  ...
  static const String chartTitle = '呼吸容量';
  ...
  @override
  State<Prs1ChartTidalVolume> createState() => _Prs1ChartTidalVolumeState();
}

class _Prs1ChartTidalVolumeState extends State<Prs1ChartTidalVolume> {
  ...
  Widget build(BuildContext context) {
    ...
    final pts = <_Pt>[];
    for (final sm in widget.bucket.rollingTidalVolume5m) {
      final v = sm.value;
      if (v == null) continue;
    ...
    final minVal = _minV(pts);

    // Some metrics (RR/MV/TV) should never go negative; keep axis baseline at 0 when applicable.
    final clampToZero = true;
    final y = _niceAxis(clampToZero ? 0.0 : minVal, maxVal);

    return LayoutBuilder(
      builder: (context, c) {
        final maxW = c.maxWidth.isFinite ? c.maxWidth : 900.0;
        final chartViewportW = math.max(0.0, maxW - _labelW);
        // keep pxPerMinute consistent with leak/pressure charts
        ...
        return Column(
          ...
          children: [
            ...
            Row(
              children: [
                const Text(
                  Prs1ChartTidalVolume.chartTitle,
                  style: TextStyle(fontSize: 22, fontWeight: FontWeight.w700),
                ),
                const SizedBox(width: 10),
                _LegendDot(color: Colors.pink.withOpacity(0.85)),
                const Spacer(),
              ],
            ),
                  ...
                  children: [
                    SizedBox(
                      ...
                      child: CustomPaint(
                        ...
                        painter: _AxisPainter(theme: theme, tickMin: y.tickMin, tickMax: y.tickMax, ticks: y.ticks, unit: 'mL'),
                      ),
                    ),
                    Expanded(
                        ...
                        child: SingleChildScrollView(
                          ...
                          child: CustomPaint(
                            ...
                            painter: _SeriesPainter(
                              ...
                              ticks: y.ticks,
                              pts: pts,
                              lineColor: Colors.pink.withOpacity(0.85),
                            ),
                          ),
                        ),
                      ),
...
double _max2(double a, double b) => math.max(a, b);
double _min2(double a, double b) => math.min(a, b);
