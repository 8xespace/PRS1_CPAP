...
class Prs1DailyAggregator {
  ...
  /// Night-bucket key (local date).
  ///
  /// CPAP therapy sessions are naturally *night based* (OSCAR style), not
  /// calendar-day based. For example, a session that runs 23:07 → 07:20
  /// should be treated as a single "night" bucket keyed by the evening date.
  ///
  /// We follow the same heuristic used by the Pressure chart axis:
  /// - If local hour < 12 (i.e., after midnight to late morning), it belongs
  ///   to the previous night's bucket.
  /// - Otherwise, it belongs to the current date bucket.
  DateTime _nightKey(DateTime local) {
    final base = local.hour < 12 ? local.subtract(const Duration(days: 1)) : local;
    return DateTime(base.year, base.month, base.day);
  }

  DateTime _nightStart2030(DateTime local) {
    final day = _nightKey(local);
    return DateTime(day.year, day.month, day.day, 20, 30);
  }
  ...
  void _bucketizeSampleEvents(
    ...
    for (final e in events) {
      ...
      final day = _nightKey(e.time.toLocal());
      final b = buckets[day];
      if (b == null) continue;
  ...
  List<Prs1DailyBucket> build(List<Prs1Session> sessions) {
    ...
    final startDay = _nightKey(minStart.toLocal());
    final endDay = _nightKey(maxEnd.toLocal());
    ...
    // 1) Build session slices per night bucket (accurate usage time).
    for (final s in sessions) {
      DateTime segStart = s.start;
      final segEndAll = s.end;

      while (segStart.isBefore(segEndAll)) {
        final day = _nightKey(segStart.toLocal());
        final nextBoundary = _nightStart2030(segStart.toLocal()).add(const Duration(hours: 24));
        final segEnd = segEndAll.isBefore(nextBoundary) ? segEndAll : nextBoundary;
...
class _MutableBucket {
  ...
  void computeDerived(Prs1DailyAggregationConfig cfg) {
    ...
    //
    // IMPORTANT:
    // - For DreamStation (F0V6), the decoder emits Prs1EventType.snore where
    //   `event.value` is the *snore count in that interval* (OSCAR: PRS1SnoreEvent).
    // - Therefore, the meaningful aggregate is the **sum of values**, not the
    //   number of snore events.
    int snSum = 0;
    for (final e in events) {
      if (e.type != Prs1EventType.snore) continue;
      final v = (e.value == null) ? 1 : e.value!.round();
      snSum += (v <= 0 ? 1 : v);
    }
    snoreCount = snSum;
    ...
    // Night-based bucket start (20:30 local) to match OSCAR-like axis.
    final bucketStartEpochSecHeat = DateTime(day.year, day.month, day.day, 20, 30).toUtc().millisecondsSinceEpoch ~/ 1000;
    final minutesHeat = 24 * 60;
    snoreHeatmap1mCounts = Prs1SnoreHeatmap.countsPerMinute(
      dayStartEpochSec: bucketStartEpochSecHeat,
      minutes: minutesHeat,
      events: events,
    );
    ...
    // Derive an OSCAR-like "unintentional leak" series.
    // DreamStation provides total leak samples. OSCAR shows both Total Leak and
    // Unintentional Leak; for our first pass we estimate baseline/intentional leak
    // as a low-percentile of total leak over the whole night, then subtract.
    if (leakSamples.isNotEmpty) {
      final vals = leakSamples
          .map((e) => e.value)
          .whereType<double>()
          .where((v) => v.isFinite)
          .toList();
      ...
      if (vals.isEmpty) {
        // Keep a sane default so UI can always draw a threshold line.
        leakBaselineLpm = null;
        largeLeakThresholdLpm = cfg.leakOverThreshold; // fallback; will be overwritten by total-leak mean (OSCAR dashed line)
        unintentionalLeakSamples..clear();
      } else {
        vals.sort();
        final idx = (0.10 * (vals.length - 1)).floor().clamp(0, vals.length - 1);
        final baseline = vals[idx];
        leakBaselineLpm = baseline;
    ...
    for (final sl in slices) {
      ...
      for (final b in breathList) {
        ...
        // Keep breath objects inside the bucket so rolling MV/RR/TV charts
        // can build time series without re-segmenting.
        breaths.add(b);
    ...
    if (tv.isNotEmpty) {
      ...
      final bucketStartEpochSec2 = DateTime(day.year, day.month, day.day, 20, 30).toUtc().millisecondsSinceEpoch ~/ 1000;
      const minutesInDay2 = 24 * 60;
      final fl1m = Prs1FlowLimitation.minuteMedianSeries(
        breathSeries: flSeries,
        dayStartEpochSec: bucketStartEpochSec2,
    ...
    final bucketStartEpochSecCorr = DateTime(day.year, day.month, day.day, 20, 30).toUtc().millisecondsSinceEpoch ~/ 1000;
    final highFlEpisodes = Prs1ValueEpisodes.fromMinuteBands(
      flowLimitationSeverityBands5mEma,
      dayStartEpochSec: bucketStartEpochSecCorr,
      bandPredicate: (b) => b >= 2,
      minDurationSec: 60,
    ...
    final bucketStartEpochSec = DateTime(day.year, day.month, day.day, 20, 30).toUtc().millisecondsSinceEpoch ~/ 1000;
    const minutes = 24 * 60;
    ...
    // Breath-derived rolling series (5-minute window).
    // These back the MV / RR / TV charts and share the same breath segmentation pipeline.
    if (breaths.isNotEmpty) {
      rollingMinuteVent5m = Prs1RollingMetrics.rollingMedianBreathMetric(
        dayStartEpochSec: bucketStartEpochSec,
        breaths: breaths,
        windowMinutes: 5,
        valueOf: (b) => b.minuteVentilationLpm,
      );

      rollingRespRate5m = Prs1RollingMetrics.rollingMedianBreathMetric(
        dayStartEpochSec: bucketStartEpochSec,
        breaths: breaths,
        windowMinutes: 5,
        valueOf: (b) => b.respRateBpm,
      );

      // Keep tidal volume in mL (OSCAR uses mL in the statistics table).
      rollingTidalVolume5m = Prs1RollingMetrics.rollingMedianBreathMetric(
        dayStartEpochSec: bucketStartEpochSec,
        breaths: breaths,
        windowMinutes: 5,
