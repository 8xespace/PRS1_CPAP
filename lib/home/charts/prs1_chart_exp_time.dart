...
class Prs1ChartExpTime extends StatefulWidget {
  const Prs1ChartExpTime({
    super.key,
  ...
  @override
  State<Prs1ChartExpTime> createState() => _Prs1ChartExpTimeState();
}

class _Prs1ChartExpTimeState extends State<Prs1ChartExpTime> {
  ...
  Widget build(BuildContext context) {
    ...
    final pts = <_Pt>[];
    for (final sm in widget.bucket.rollingExpTime5m) {
      final v = sm.value;
      if (v == null) continue;
    ...
    // - OSCAR 截圖：吐氣時間 Y 軸上限約 6.0 秒
    // - 仍保護性：若資料超過 6，改用自動 nice axis 避免截斷。
    final maxVal = _maxV(pts);
    final y = _fixedAxis(
      fixedMax: 6.0,
      fixedTicks: const [0.0, 2.0, 4.0, 6.0],
    ...
    return LayoutBuilder(
      builder: (context, c) {
        ...
        // keep pxPerMinute consistent with MV/RR/TV/leak/pressure charts
                        ...
                        child: SingleChildScrollView(
                          ...
                          child: CustomPaint(
                            ...
                            painter: _SeriesPainter(
                              ...
                              ticks: y.ticks,
                              pts: pts,
                              lineColor: Colors.green.withOpacity(0.85),
                            ),
                          ),
                        ),
                      ),
